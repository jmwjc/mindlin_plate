...
elements_w, elements_Γ = Tri3toTRTri3(elements,elements_Γ)

nʷ = length(elements_Γ)
nᵠ = length(nodes)
nᵛ = nᵠ
kʷʷ = zeros(nʷ,nʷ)
kᵠᵠ = zeros(2*nᵠ,2*nᵠ)
...
@timeit to "calculate ∫κκdΩ" begin
    ...
    𝑎ᵛᵠ = ∫QφdΩ=>elements_φ
    𝑎ᵛᵛ = ∫QQdΩ=>elements_φ
    𝑎ᵛʷ = ∫Q∇wdΩ=>(elements_φ,elements_w)
    𝑓ʷ = ∫wqdΩ=>elements_w
    𝑓ᵠ = ∫φmdΩ=>elements_φ
...
@timeit to "calculate ∫αwwdΓ" begin
    @timeit to "get elements" elements_1_φ = getElements(nodes, entities["Γ¹"], integrationOrder, normal=true)
    @timeit to "get elements" elements_2_φ = getElements(nodes, entities["Γ²"], integrationOrder, normal=true)
    @timeit to "get elements" elements_3_φ = getElements(nodes, entities["Γ³"], integrationOrder, normal=true)
    @timeit to "get elements" elements_4_φ = getElements(nodes, entities["Γ⁴"], integrationOrder, normal=true)
    elements_1_w = Seg2toTRTri3(elements_1_φ,elements_w)
    elements_2_w = Seg2toTRTri3(elements_2_φ,elements_w)
    elements_3_w = Seg2toTRTri3(elements_3_φ,elements_w)
    elements_4_w = Seg2toTRTri3(elements_4_φ,elements_w)
    prescribe!(elements_1_w, :α=>1e8*E, :g=>w)
    prescribe!(elements_2_w, :α=>1e8*E, :g=>w)
    prescribe!(elements_3_w, :α=>1e8*E, :g=>w)
    prescribe!(elements_4_w, :α=>1e8*E, :g=>w)
    prescribe!(elements_1_φ, :α=>1e8*E, :g₁=>φ₁, :g₂=>φ₂, :n₁₁=>1.0, :n₁₂=>0.0, :n₂₂=>1.0)
    prescribe!(elements_2_φ, :α=>1e8*E, :g₁=>φ₁, :g₂=>φ₂, :n₁₁=>1.0, :n₁₂=>0.0, :n₂₂=>1.0)
    prescribe!(elements_3_φ, :α=>1e8*E, :g₁=>φ₁, :g₂=>φ₂, :n₁₁=>1.0, :n₁₂=>0.0, :n₂₂=>1.0)
    prescribe!(elements_4_φ, :α=>1e8*E, :g₁=>φ₁, :g₂=>φ₂, :n₁₁=>1.0, :n₁₂=>0.0, :n₂₂=>1.0)
...
@timeit to "calculate error" begin
    ...
    elements_w, elements_Γ, nodes = Tri3toTRTri3(elements,elements_Γ)
