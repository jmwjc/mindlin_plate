...
# Helper: resolve Γ-related physical names in entities dict using node coordinates
function resolve_Gamma!(entities::Dict{String,Pair{Int,Vector{Int}}}, getNodesFn, R::Float64)
    # find keys that start with the Greek letter Γ
    gamma_keys = [k for k in keys(entities) if startswith(k, "Γ")]
    if isempty(gamma_keys)
        # nothing to do
        return
    end
    # for each gamma key compute average radius of nodes on its entities
    avg_radii = Dict{String,Float64}()
    for k in gamma_keys
        p = entities[k]
        # try to get elements on this physical group and compute mean radius of control points
        try
            # getElements will return element objects whose control points give node coords
            elms = getElements(getNodesFn(), p, 1)
            rs = Float64[]
            for elm in elms
                for c in elm.𝓒
                    push!(rs, hypot(c.x, c.y))
                end
            end
            avg_r = isempty(rs) ? 0.0 : mean(rs)
            avg_radii[k] = avg_r
        catch err
            @warn "resolve_Gamma!: failed to probe elements for physical group $k: $err"
            avg_radii[k] = 0.0
        end
    end
    # choose the key with largest average radius as the circular outer boundary
    circ_key = findmax(collect(values(avg_radii)))[2] >= 0 ? begin
        # findmax returns (value, index) but we need the corresponding key
        vals = collect(values(avg_radii))
        ks = collect(keys(avg_radii))
        ks[argmax(vals)]
    end : gamma_keys[1]
    # set a composite Γ that combines all Γ* groups (concatenate entity id vectors)
    combined_entities = Int[]
    for k in gamma_keys
        append!(combined_entities, entities[k][2])
    end
    # overwrite/insert entries to be used later in the code
    entities["Γ"] = Pair(1, combined_entities)
    entities["Γ_circ"] = entities[circ_key]
    @info "resolve_Gamma!: mapped Γ keys: " * join(gamma_keys, ", ") * "; chosen Γ_circ = $circ_key"
end
...
XLSX.openxlsx("xls/circular_16_tri3_16.xlsx", mode="w") do xf
    for ndiv = ndiv_w:32
        ...
        # resolve Γ names in entities_w (make entities_w["Γ"] and entities_w["Γ_circ"] available)
        resolve_Gamma!(entities_w, () -> nodes_w, R)
        ...
        # resolve Γ names in entities_φ
        resolve_Gamma!(entities_φ, () -> nodes_φ, R)
        @timeit to "calculate support domain" begin
        ...
        # resolve Γ names in entities (combine Γ* and pick outer Γ_circ)
        resolve_Gamma!(entities, () -> nodes, R)
        ...
        @timeit to "calculate ∫MMdΩ ∫MφdΩ" begin
            ...
            @timeit to "get elements" begin
                # collect original Γ* keys (exclude synthesized entries added by resolve_Gamma!)
                gamma_keys = [k for k in keys(entities) if startswith(k, "Γ") && !(k in ("Γ", "Γ_circ"))]
                elements_m_Γ = ApproxOperator.AbstractElement[]
                if isempty(gamma_keys) && haskey(entities, "Γ")
                    # fallback: use combined Γ if no individual keys available
                    temp = getPiecewiseBoundaryElements(entities["Γ"], entities["Ω"], eval(type_M), integrationOrder)
                    append!(elements_m_Γ, temp)
                else
                    # call package function per original Γ* group and concatenate results to avoid internal division issues
                    for k in gamma_keys
                        try
                            temp = getPiecewiseBoundaryElements(entities[k], entities["Ω"], eval(type_M), integrationOrder)
                            append!(elements_m_Γ, temp)
                        catch err
                            @warn "getPiecewiseBoundaryElements failed for physical group $k: $err"
                        end
                    end
                end
            end
