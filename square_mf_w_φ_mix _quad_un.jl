...
sᵠ = 1.5
for ndiv_w = 10:25
 XLSX.openxlsx("xls/square_un_$(ndiv_φ)_quad4_$(ndiv_w).xlsx", mode="w") do xf
...
push!(nodes_w, :s₁=>s, :s₂=>s, :s₃=>s)
# ─── Rotation Φ ───────────────────────────────────────────
@timeit to "open msh file" gmsh.open("msh/patchtest_un_quad4_$ndiv_φ.msh")
...
push!(nodes_φ, :s₁=>s, :s₂=>s, :s₃=>s)
# ─── Shear ────────────────────────────────────────────────
@timeit to "open msh file" gmsh.open("msh/patchtest_un_quad4_$ndiv.msh")
